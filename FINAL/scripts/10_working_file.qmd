...
```{r}
# load data from csv
num_train_scaled_label <- read.csv("../data/num_train_scaled_label.csv")
num_train_scaled <- num_train_scaled_label[, -21]

summary(num_train_scaled_label)
```
...
```{r KMO}
kmo_result <- KMO(num_train_scaled)
kmo_result$MSA
#KMO_result$Image
kmo_result$MSAi

round(min(kmo_result$MSAi),2)
colnames(num_train_scaled)[which.min(kmo_result$MSAi)]
...
kmo_summary <- num_train_scaled %>% 
  KMO() %>% 
  .$MSAi %>% 
  tibble::enframe() %>% 
  mutate(name = colnames(num_train_scaled), 
         MSAi_band = cut(value, 
...
paran(num_train_scaled, iterations = 5000)
```
...
pca_num_train_scaled <- dudi.pca(num_train_scaled,scannf = F, nf=4)

scree <- fviz_screeplot(pca_num_train_scaled)
eigen <- get_eigenvalue(pca_num_train_scaled)

pca1 <- fviz_pca_biplot(pca_num_train_scaled, label="var",var.lab = TRUE, col.ind = "#D95F02", alpha.ind = 0.4, col.var = "grey30", repel=TRUE)  +
  ggtitle("PCA Biplots (dimensions 1x2, 2x3) of scaled dataset")

pca2 <- fviz_pca_biplot(pca_num_train_scaled,label="var", col.ind = "#1B9E77", alpha.ind = 0.4, col.var = "grey30", repel=TRUE,axes=c(2,3)) +
  ggtitle("")

pca3 <- fviz_pca_biplot(pca_num_train_scaled, col.ind="x", col.var = "grey30", label="var", repel=TRUE,axes=c(1,2)) +
      scale_color_gradient2(low="white", mid="blue",
      high="red", midpoint=0.6)

pca4 <- fviz_pca_biplot(pca_num_train_scaled,
                        repel = T,
                        label = "var",
                        col.ind = train$label) + 
  theme(legend.position = "none")

pca5 <- fviz_pca_ind(pca_num_train_scaled,
                     geom.ind = "point", # show points only (but not "text")
                     col.ind = num_train_scaled_label$label, # colour by groups
                     addEllipses = TRUE,
...
```{r euclid_distance}
# euclidean distance
dist_euc <-get_dist(num_train_scaled,method="euclidean")
...
# plot
cluster_5 <- fviz_cluster(list(data = num_train_scaled, cluster = res_cluster), 
...
labels <- rownames(num_train_scaled)
...
KM3 <- kmeans(num_train_scaled, 3)
KM4 <- kmeans(num_train_scaled, 4)
KM5 <- kmeans(num_train_scaled, 5)

# create a color palette with the same number of colors as clusters
...
par(mfrow = c(1, 3))
plot(num_train_scaled[, 1:2], col = col3[KM3$cluster], pch = KM3$cluster,
     xlab = "", ylab = "PC2", main = "k-means (k = 3)")
plot(num_train_scaled[, 1:2], col = col4[KM4$cluster], pch = KM4$cluster,
     xlab = "PC1", ylab = "", main = "k-means (k = 4)")
plot(num_train_scaled[, 1:2], col = col5[KM5$cluster], pch = KM5$cluster,
...
fviz_nbclust(num_train_scaled,FUN=hcut,method="wss")
```
...
```{r silhouette_clust}
fviz_nbclust(num_train_scaled, FUN = hcut, method = "silhouette")
```
...
```{r cluster_KM}
fviz_cluster(KM4,data = num_train_scaled, repel = TRUE, geom = "point", label = "none")
fviz_cluster(KM5, data = num_train_scaled, repel = TRUE, geom = "point", label = "none")
...
sil <- sapply(2:10, function(k) {
  km <- kmeans(num_train_scaled, centers = k)
...
fviz_nbclust(num_train_scaled, kmeans, method = "wss") + 
...
gap_stat <- clusGap(num_train_scaled, FUN = kmeans, K.max = 10, B = 50, verbose = FALSE)
...
var_contrib <- data.frame(
  Variable = names(num_train_scaled),
